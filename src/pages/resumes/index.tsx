...
type ColumnType = ColumnDef<ResumeInterface, unknown>;
...
export function ResumeListPage(props: ResumeListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ResumeInterface>>(
    () => `/resumes?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ResumeInterface) => {
    if (hasAccess('resume', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/resumes/view/${row.id}`);
    }
  };
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="resume" />
        </Flex>

        {hasAccess('resume', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/resumes/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
