...
function DashboardPage() {
  ...
  const templateStat = useSWR<PaginatedInterface<TemplateInterface>>(
    () => `/templates`,
    () => getTemplates({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Templates',
      result: templateStat?.data?.totalCount,
      isLoading: templateStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Jobs',
